...
// will send to the indexer queue the missing car file, using the cache to avoid to send the same car multiple times
...
async function recoverV0Tables(car, queue = 'indexer-topic') {
  try {
    ...
    sqsClient.send(new SendMessageCommand({ QueueUrl: queue, MessageBody: `{"body":"${car}",skipExists:true}` }))
